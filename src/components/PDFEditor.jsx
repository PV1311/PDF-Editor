...
function PDFEditor() {
  ...
  function handleBlur() {
    if (eraseRects.some((rect) => rect.page === pageNumber && !rect.erased)) {
      setEraseRects((prev) =>
        prev.filter((rect) => rect.page !== pageNumber || rect.erased)
      );
  ...
  function handleEraseArea(id) {
    setEraseRects((prev) =>
      prev.map((rect) => (rect.id === id ? { ...rect, erased: true } : rect))
    );
    setEraseMode(false);
  }
        ...
        >
          {hoveredEraseId === rect.id && (
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                setEraseRects((prev) => prev.filter((r) => r.id !== rect.id));
          ...
          .forEach((box) => {
            ...
            textLines.forEach((line, i) => {
              const y =
                (cssPixelHeight -
                  box.top -
                  fontSizePt * (i + 1) * lineHeightRatio) *
                scaleY;
    ...
    <div className="container mx-auto p-4">
      ...
      {pdfFile && (
        <div className="max-w-[calc(100%-20rem)] mx-auto">
          <div className="toolbar bg-gray-500 p-4 mb-4 rounded flex gap-4 justify-center">
            {addTextMode || activeTextBox ? (
              <TextStyleToolbar />
            ) : (
              <>
          ...
          >
            ...
            <canvas
              ...
              style={{
                ...
                left: 0,
                pointerEvents:
                  eraseMode || (addTextMode && !activeTextBox)
                    ? "auto"
                    : "none",
                zIndex: 10000,
                width: pageDims.width,
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      lineHeight: box.lineHeight || 1.2,
                      pointerEvents:
                        dragging && draggedBoxId === box.id ? "none" : "auto",
                      userSelect: "none",
                      maxWidth: 200,
                      overflowWrap: "break-word",
                      cursor:
                        dragging && draggedBoxId === box.id
                          ? "grabbing"
                          : "grab",
                      whiteSpace: "pre-wrap",
                    }}
